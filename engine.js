...
(function () {
  ...
  var STORE_KEY = "sbg_route1_team_" + CFG.name;
  ...
  function resetProgress(confirmMsg) {
  ...
  function renderPassage() {
    ...
    PASSAGE.parts.forEach(function (part) {
      if (typeof part === "string") {
        ...
        var ansLen = (CP[part.cp] && CP[part.cp].answer ? CP[part.cp].answer.length : 8);
        var blankWidth = "style=\"width:" + (ansLen + 2) + "ch\"";
        html += '<input class="blank" type="text" autocomplete="off" autocapitalize="off" spellcheck="false" maxlength="' + (ansLen + 4) + '" ' + blankWidth + ' data-blank="' + part.blank + '" value="' + esc(val) + '" placeholder="' + part.blank + '">';
      }
    });
    html += "</div>";
